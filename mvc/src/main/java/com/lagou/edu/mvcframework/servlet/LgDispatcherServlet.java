...
public class LgDispatcherServlet extends HttpServlet {
    ...
    private List<String> classNames = new ArrayList<>();//缓存扫描导的类的全限定类名

    //handlerMapping
    //private Map<String, Method> handlerMapping = new HashMap<String, Method>();//存储url和method之间的映射关系
...
//根据uri获取到能够处理当前请求的hanlder（从handlermapping中（list）)
        ...
        //参数绑定
        //获取所有参数类型数组，这个数组的长度就是我们最后要传入args数组的长度
    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        //5.构造一个HandlerMapping处理器映射器，将配置好的url和Method 建立映射关系
        initHandlerMapping();

        System.out.println("拉钩Springmvc初始化完成");
        //6.等待请求进入处理请求
    }
...
//处理计算方法的位置参数信息 //HttpServletRequest request, HttpServletResponse response,String name
    ...
    //基于classNames缓存的类的全限定类名，以及反射技术完成对象创建和管理
    private void doInstance()  {
        ...
        try {
            for (int i = 0; i < classNames.size(); i++) {
                ...
                if (aClass.isAnnotationPresent(LagouController.class)) {
                    //controller的id此处不做过多处理，不取value，就拿类的首字母小写作为id，保存到我们的ioc容器
    ...
    //扫描类
    //scanPackage:com.lagou.demo---->磁盘上的文件夹（File）
